...
public:
    ...
    bool fruit_eaten(Position fruit_pos) {
        if (segments[0].x == fruit_pos.x && segments[0].y == fruit_pos.y) return true;
        return false;
    }

    bool hit_wall() {
        if (segments[0].x == 0 || segments[0].x == WIDTH) return true;
        if (segments[0].x == 0 || segments[0].y == HEIGHT) return true;
...
public:
    ...
    bool is_opposite(int curr_dir, int new_dir) { // checks if input tries to make snake in the opposite direction (impossible)
