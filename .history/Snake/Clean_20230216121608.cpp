...
public:
    ...
    void write_to_grid(Position point, const char *element) {
        int x      = point.x;
        int y      = point.y;
        grid[y][x] = element;
    }
    ...
    void loop() {
        ...
        write_to_grid(new_fruit, FRUIT_CHAR);
        draw();
        ...
        while ((input = getch()) != 'q') {
            ...
            if (new_dir != ERR) {
                ...
                write_to_grid(new_fruit, FRUIT_CHAR.c_str());
    ...
    void draw() {
        ...
        mvprintw(21, 50, "By: Jonathan (q to quit)");
