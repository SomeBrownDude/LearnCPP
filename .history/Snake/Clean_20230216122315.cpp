...
const int DELAY           = 90;  // How fast the snake moves (lower = faster)
...
public:
    ...
    void draw() {
        ...
        attron(A_BOLD);
        mvprintw(HEIGHT/2, WIDTH + 10, "Snake Game");
        mvprintw(HEIGHT/2 + 1, WIDTH + 10, "By: Jonathan (q to quit)");
        mvprintw(HEIGHT/2 + 3, WIDTH + 9, "Score: ");
        mvprintw(HEIGHT/2 + 4, 58, to_string(snake.snake_pos().size() - 1).c_str());
        attroff(A_BOLD);
        refresh();
    }
